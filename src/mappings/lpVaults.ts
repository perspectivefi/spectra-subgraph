import { LPVaultDeployed, RegistryUpdated } from "../../generated/LPVaultFactory/LPVaultFactory";
...
import { getName, getSymbol, getTotalAssets, getUnderlying } from "../entities/LPVault";
import { getAsset } from "../entities/Asset";
import { AssetType } from "../utils";
import { Deposit, FeeUpdated, Paused, PoolIndexUpdated, Unpaused, Withdraw } from "../../generated/LPVault/LPVault";
...
export function handleLPVaultDeployed(event: LPVaultDeployed): void {
    ...
    let underlying = getAsset(underlyingAddress.toHex(), event.block.timestamp, AssetType.UNDERLYING);
...
export function handlePoolIndexUpdated(event: PoolIndexUpdated): void {
    ...
    let poolAddress = getPool(Address.fromString(lpVault.lpVaultFactory), Address.fromString(lpVault.future), event.params._newIndex);
...
export function handleDeposit(event: Deposit): void {
    ...
    if (lpVault) {
        let underlyingAddress = getUnderlying(Address.fromString(lpVault.future));
